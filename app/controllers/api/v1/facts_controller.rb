  ...
  "The fact entry could not be created. #{@fact.errors.full_messages.to_sentence}"},
    ...
    def update
        if @fact.update(fact_params)
            render json: @member.fact
        else
            render json: { error: "The fact entry could not be updated. #{@fact.errors.full_messages.to_sentence}", status: :unprocessable_entity}
    end
    ...
    def destroy
        if @fact.destroy
            ...
            render json: { error: 'The fact could not be deleted.' }, status: :unprocessable_entity
          end
    end
